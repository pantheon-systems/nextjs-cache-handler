...
describe('TagsBuffer', () => {
  ...
  describe('awaitDurable', () => {
    it('should not resolve until the queued update has been written', async () => {
      const buf = createBuffer(1000);

      // Prime the buffer so it is inside its rate-limit window -- an idle
      // buffer schedules with zero delay and would flush on the next tick.
      buf.addTags('key0', ['tag0']);
      ...
      let resolved = false;
      const durable = buf.awaitDurable().then(() => {
        resolved = true;
      });

      // Before the scheduled flush fires, the mapping is still only in memory
      ...
      expect(mockWrite).toHaveBeenCalledTimes(2);
      // mockRead hands back the same object each call, so it accumulates.
      expect(mockWrite).toHaveBeenLastCalledWith({ tag0: ['key0'], tag1: ['key1'] });
    ...
    it('should resolve immediately when nothing is pending', async () => {
      ...
      await buf.awaitDurable();

      expect(mockWrite).not.toHaveBeenCalled();
    });

    it('should wait for the scheduled flush rather than forcing an extra write', async () => {
      // The whole point of the buffer is one write per interval per object;
      // awaiting durability must not defeat that coalescing.
      const buf = createBuffer(1000);

      buf.addTags('key1', ['tag1']);
      const a = buf.awaitDurable();
      buf.addTags('key2', ['tag2']);
      const b = buf.awaitDurable();

      await vi.advanceTimersByTimeAsync(1100);
      await Promise.all([a, b]);

      expect(mockWrite).toHaveBeenCalledTimes(1);
      expect(mockWrite).toHaveBeenCalledWith({ tag1: ['key1'], tag2: ['key2'] });
    });

    it('should resolve once a failed write succeeds on retry', async () => {
      mockWrite.mockRejectedValueOnce(new Error('Rate limited'));
      ...
      // First attempt fails; the update goes back on the queue
      await vi.advanceTimersByTimeAsync(1100);
      expect(buf.pendingCount).toBe(1);
      expect(resolved).toBe(false);

      // Retry (scheduled at 2x the interval) succeeds
      await vi.advanceTimersByTimeAsync(2100);
      await durable;
    ...
    it('should give up after the durability timeout instead of hanging', async () => {
      mockWrite.mockRejectedValue(new Error('GCS down'));

      buffer = new TagsBuffer({
        flushIntervalMs: 1000,
        durabilityTimeoutMs: 3000,
        readTagsMapping: mockRead,
        writeTagsMapping: mockWrite,
        handlerName: 'TestBuffer',
      });
      ...
      let resolved = false;
      const durable = buffer.awaitDurable().then(() => {
        resolved = true;
      });
      ...
      // Resolves (never rejects) so the caller's cache write isn't reported as
      // failed, while the update stays queued for further retries.
      expect(resolved).toBe(true);
      expect(buffer.pendingCount).toBe(1);
    });

    it('should release awaiters on destroy rather than waiting out the timeout', async () => {
      const buf = createBuffer(1000);
      buf.addTags('key1', ['tag1']);

      const durable = buf.awaitDurable();
      buf.destroy();
